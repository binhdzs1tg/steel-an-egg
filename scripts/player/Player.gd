...
##
## FIXES:
##   - First-person previously set spring_arm.visible = false, which hid the
##     camera child along with the arm (risk of a black screen) and manually
##     fought the SpringArm by overwriting the camera transform every frame.
##     First-person now just collapses the spring length to 0.
##   - The interaction raycast + prompt signals ran 60x per second; now
##     throttled to 10x/s and only emits when the prompt actually changes.
##   - Kill-plane: falling off the world respawns the player at the Base
...
const KILL_PLANE_Y: float = -25.0
const SPAWN_POINT: Vector3 = Vector3(0, 1.2, 3.0)
const INTERACTION_INTERVAL: float = 0.1
...
func _physics_process(delta: float) -> void:
	# Kill-plane: falling off the world returns the player to the Base.
	if global_position.y < KILL_PLANE_Y:
		global_position = SPAWN_POINT
		velocity = Vector3.ZERO
		NotificationSystem.notify("You fell off the world! Returned to Base.", "warning", 2.0)
		return
	...
	# Throttled interaction check (10x/s instead of 60x/s)
	_interaction_timer -= delta
	if _interaction_timer <= 0.0:
		_interaction_timer = INTERACTION_INTERVAL
		_update_interaction()
...
func _update_camera(delta: float) -> void:
	...
	match camera_mode:
		CameraMode.FIRST_PERSON:
			# Collapse the spring arm instead of hiding it (hiding the arm also
			# hid the camera child -> black screen). The camera then sits at the
			# head position while the body model is hidden.
...
func _update_interaction() -> void:
	...
	if result and not result.collider.is_queued_for_deletion() and result.collider.has_method("interact"):
		...
		prompt = "[E] " + (result.collider.get_prompt_text() if result.collider.has_method("get_prompt_text") else "Interact")
	interaction_target = new_target
	# Only touch the Label3D / emit signals when the state actually changed.
	if prompt != _last_prompt_text:
		_last_prompt_text = prompt
		interact_prompt_label.text = prompt
	var now_visible := not prompt.is_empty()
	if now_visible != _prompt_visible:
		_prompt_visible = now_visible
		interact_prompt_label.visible = now_visible
		interaction_prompt_changed.emit(prompt, now_visible)
