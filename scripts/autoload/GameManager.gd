...
##
## PERF FIX: tick() used to run EVERY frame (60x/s) and did:
##   try_unlock_biome_by_speed() + AchievementSystem.check_all() + an
##   add_speed_xp() call (which emitted speed_changed and re-created a HUD
##   tween every single frame). Everything expensive now runs on a 4 Hz
##   scheduler instead, and money income is flushed as ONE batched
##   add_money() per tick instead of one call per $1 per frame.
...
const SYSTEM_TICK_INTERVAL: float = 0.25  # 4 Hz game-system tick
...
var _system_timer: float = 0.0
var _pending_income: float = 0.0
const TREADMILL_XP_TABLE: Array = [1, 2, 4, 7, 12, 20, 33, 55, 90, 150, 250, 415, 690, 1150, 1900, 3150, 5200, 8600, 14200, 23500]
...
# Called on the slow system tick (and once at boot)
...
func tick(delta: float) -> void:
	# Income accumulates every frame (cheap float add) ...
	_pending_income += Economy.accumulate_income(delta)
	_system_timer += delta
	if _system_timer < SYSTEM_TICK_INTERVAL:
		return
	_system_timer -= SYSTEM_TICK_INTERVAL
	if _system_timer >= SYSTEM_TICK_INTERVAL:
		# Safety: don't spiral if a huge delta happened (e.g. breakpoint).
		_system_timer = 0.0

	# ... then flush income as ONE batched add_money per system tick.
	var earned := int(_pending_income)
	if earned > 0:
		_pending_income -= float(earned)
		Economy.add_money(earned)
	Economy.emit_income_rate()

	# Treadmill passive XP (batched at 4 Hz instead of per-frame)
	var treadmill_level := Economy.get_upgrade_level("treadmill")
	if treadmill_level > 0:
		var xp_per_sec := _treadmill_xp_for_level(treadmill_level)
		Economy.add_speed_xp(int(xp_per_sec * SYSTEM_TICK_INTERVAL))
...
func _treadmill_xp_for_level(level: int) -> int:
	if level - 1 < TREADMILL_XP_TABLE.size():
		return int(TREADMILL_XP_TABLE[level - 1])
	return int(TREADMILL_XP_TABLE[-1])
